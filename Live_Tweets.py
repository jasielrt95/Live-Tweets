...
# This is a Dict of Bounding Boxes from graydon, in Github
...
# This part of the code read your keys from a txt file called Keys.txt
with open("keys.txt", 'r') as reader:
    keys = reader.read().splitlines()

# Here we assing the keys found on the Keys.txt file to some variables.

Twitter_ApiKey = keys[0]

# Credential Secret Key
Twitter_SecretKey = keys[1]
...
auth = tweepy.OAuthHandler(consumer_key=Twitter_ApiKey, consumer_secret=Twitter_SecretKey)

auth.set_access_token(Access_token, Access_token_Secret)


'''Create API call variable'''
...
# This is a list of posible keywords that we are looking for in the tweets
search = []
# Here the user will specify wich topics he wants to look for
topic = (input("\n What topic's do you want to look for (enter as many as you want, separated by ',': "))
search = topic.split(',')

# Here the user will specify wich language to use for searching tweets 
language = input("\n In what language do you want to search for tweets: ")

# Here the user will specify wich country to use for searching tweets 
country = input("\n In wich country do you want to search for tweets: ")
country = country_bounding_boxes.get(country)
country = str(country[1])
country = country.replace('(','').replace(')','')
country = country.split(',')


# Listener Class to view tweets on realtime
class TweetsListener(tweepy.StreamListener):

    # Let me know if conected
    def on_connect(self):
        print("Connected: ")

    # display tweets on terminal
    def on_status(self, status):
        i = 0
        if any(x in status.text for x in search):
            i = i + 1
            # If the tweet is small then we use status.text
            if not status.truncated:
                with open("Tweets.txt", "a", encoding='utf-8') as w:
                    text = status.text
                    w.write("Tweet: " + ": " + text + "\n")
            # If the tweet is big, then we use status.extended_tweet["full_text"]
            else:
                text = status.extended_tweet["full_text"]
                with open("Tweets.txt", "a", encoding='utf-8') as w:
                    w.write("Tweet: " + ": " + text + "\n" + "\n")
            print("Tweet by: " + str(status.user.screen_name) + "\n content: " + text + "\n")

    # Throws an error if it could not connect
...
streamingApi = tweepy.Stream(
    auth=Twitter_API.auth, listener=stream, tweet_mode="extended"
)

# Change this bounding box for a specific region.
streamingApi.filter(
    locations=[float(country[0]),float(country[1]),float(country[2]),float(country[3])],
    languages=[language],
)
